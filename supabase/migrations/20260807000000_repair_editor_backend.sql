...
alter table public.projects add column if not exists created_at timestamptz default timezone('utc', now());
...
drop policy if exists "Public can read seed projects" on public.projects;
create policy "Public can read seed projects"
on public.projects for select
to anon, authenticated
using (user_id is null and profile_id is null);
...
drop policy if exists "Public can inspect assets bucket" on storage.buckets;
create policy "Public can inspect assets bucket"
on storage.buckets for select
to anon, authenticated
using (id = 'assets');

drop policy if exists "Public can read editor assets" on storage.objects;
create policy "Public can read editor assets"
...
select
  (select count(*) from public.projects where user_id is null and profile_id is null) as public_seed_projects,
  exists(select 1 from storage.buckets where id = 'assets' and public = true) as public_assets_bucket,
  (select count(*) from pg_policies where schemaname = 'public' and tablename = 'projects') as project_policy_count,
  (select count(*) from pg_policies where schemaname = 'storage' and tablename in ('buckets', 'objects')) as storage_policy_count;
