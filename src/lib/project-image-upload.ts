...
async function getAuthenticatedUserId(): Promise<{
  ...
  if (sessionResult.error) {
    throw new Error(`无法读取登录状态：${sessionResult.error.message}`);
  }
  ...
  if (!userId) {
    ...
    if (anonymousResult.error || !anonymousResult.data.user) {
      throw new Error(
        "上传图片需要登录。请在 Supabase Auth 中启用匿名登录，或先登录后重试。",
      );
    }
...
export async function uploadProjectImage({
  ...
  const uploadResult = await client.storage.from("assets").upload(storagePath, file, {
    cacheControl: "300",
