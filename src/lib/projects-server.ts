...
function isProjectRow(value: unknown): value is ProjectRow {
  return (
    ...
    (value.source_project_id === null ||
      typeof value.source_project_id === "string")
  );
}
  ...
  try {
    ...
    try {
      const result = await supabase
        .from("projects")
        .select(
          "id, name, thumbnail_url, layers_json, created_at, updated_at, user_id, source_project_id",
        )
      ...
      const ownedSourceIds = new Set(
        projects.flatMap((project) =>
          project.userId && project.sourceProjectId
            ? [project.sourceProjectId]
            : [],
        ),
      );
      const visibleProjects = projects.filter(
        (project) => project.userId !== null || !ownedSourceIds.has(project.id),
      );
      return visibleProjects.length > 0 ? visibleProjects : [...SEED_PROJECTS];
    } finally {
